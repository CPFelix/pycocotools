...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm'):
        ...
        self.pycocotools_detect_npy = -np.ones((67000, 9))
        self.pycocotools_obj_index = -1
        ...
        if not len(ious)==0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    # 写入pycocotools_detect_npy
                    if (tind == 0) and (aRng == [0, 10000000000.0]) and (maxDet == 100):
                        self.pycocotools_obj_index += 1
                        # print("pycocotools_obj_index:{} iouThr:{} imgId:{} catId:{}".format(self.pycocotools_obj_index, t, imgId, catId))
                        self.pycocotools_detect_npy[self.pycocotools_obj_index][0] = imgId
                        box = [d["bbox"][0], d["bbox"][1], d["bbox"][0] + d["bbox"][2], d["bbox"][1] + d["bbox"][3]]
                        self.pycocotools_detect_npy[self.pycocotools_obj_index][1:5] = box
                        self.pycocotools_detect_npy[self.pycocotools_obj_index][5] = d["score"]
                        self.pycocotools_detect_npy[self.pycocotools_obj_index][6] = d["category_id"]
                    ...
                    # 写入pycocotools_detect_npy
                    if (tind == 0) and (aRng == [0, 10000000000.0]) and (maxDet == 100):
                        self.pycocotools_detect_npy[self.pycocotools_obj_index][7] = dtm[0][dind]
                        self.pycocotools_detect_npy[self.pycocotools_obj_index][8] = iou
        else:
            # print("111111")
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
    ...
    def plot_mc_curve(self, px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric', max_indx=300):
        # Metric-confidence curve
        fig, ax = plt.subplots(1, 1, figsize=(9, 6), tight_layout=True)
        if 0 < len(names) < 21:  # display per-class legend if < 21 classes
            for i, y in enumerate(py):
                ax.plot(px, y, linewidth=1, label=f'{names[i]}')  # plot(confidence, metric)
                plt.plot(px[max_indx], y[max_indx], 'rs')
                show_max = '[' + str(round(px[max_indx], 2)) + ',' + str(round(y[max_indx], 2)) + ']'
                plt.annotate(show_max, xytext=(px[max_indx], y[max_indx]), xy=(px[max_indx], y[max_indx]))
        else:
    ...
    def PR_Curve(self, names, val_json, save_dir, max_indx=300):
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        gt_num = 0
        gt_class_num_list = np.zeros(len(self.params.catIds))
        with open(val_json, "r", encoding="utf-8") as f:
            json_content = json.load(f)
            gt_num = len(json_content["annotations"])
            # print("gt_num:{}".format(gt_num))
            for ann in json_content["annotations"]:
                gt_class_num_list[ann["category_id"] - 1] += 1
        pycocotools_index = np.where(self.pycocotools_detect_npy[:, 0] == -1.0)[0]
        pycocotools_filtered_boxs = np.delete(self.pycocotools_detect_npy, pycocotools_index, axis=0)
        ...
        ######################PR曲线-yolov8########################
        # P曲线
        self.plot_mc_curve(px, p, save_dir + 'P_curve.png', names, ylabel='Precision', max_indx=max_indx)
        # R曲线
        self.plot_mc_curve(px, r, save_dir + 'R_curve.png', names, ylabel='Recall', max_indx=max_indx)
        # F1
        f1 = 2 * p * r / (p + r + 1e-16)
        self.plot_mc_curve(px, f1, save_dir + 'F1_curve.png', names, ylabel='F1', max_indx=max_indx)

    def accumulate(self, p = None, npy_file = ""):
        '''
        ...
        '''
        if npy_file != "":
            # 保存pycocotools_detect_npy
            np.save(npy_file, self.pycocotools_detect_npy)
